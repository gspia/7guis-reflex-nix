...
circleDrawer = divN $ mdo
    ...
    state ← (pure . fmap (foldStack initialState updateState)) stack
    selectedCircle ← (pure . fmap getSelected) state
    circles ← (pure . fmap circlesMap) state
    enableUndo ← (pure . fmap undoPossible) stack
    enableRedo ← (pure . fmap redoPossible) stack
